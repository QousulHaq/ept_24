...
function DraftTable({ table_data, showed_data, color, table_action, pathDetail, pathEdit, handleOpenDelete, handleSelectParticipant, selectedParticipants, status, isSearchBar }) {
    ...
    return (
        <>
            {
                isSearchBar && <SearchBar handleLoadingData={(status) => setLoadingSearch(status)} handleSearchedData={(value) => setData(value)} />
            }
                ...
                <Table sx={{ minWidth: 650 }} aria-label="simple table">
                    <TableHead className={color ? `tw-bg-${color}` : `tw-bg-neutral5`} >
                        <TableRow>
                            ...
                            {[1, 2, 3, 4, 5, 6, 7].includes(table_action) && (
                    ...
                    <TableBody>
...

                        {
                            loadingSearch ?
                                (
                                    <TableRow>
                                        <TableCell component="th" scope="row" colSpan={6} sx={{ textAlign: 'center' }}><CircularProgress /></TableCell>
                                    </TableRow>
                                ) : (
                                    data?.data.map((item, index) => (
                                        ...
                                        >
                                            <TableCell component="th" scope="row">{data?.from + index}</TableCell>
                                                    ...
                                                    showed_data.includes(Object.keys(item)[index]) && (
                                                        ...
                                                        (
                                                            moment(value, moment.ISO_8601, true).isValid() ?
                                                                <TableCell component="th" scope="row" key={index}>{moment(value).format('ddd, DD MMM YYYY')}</TableCell>
                                                                :
...

                                            {
                                                ...
                                                <TableCell align='center'>
                                                    <div className="action-button-wrapper tw-w-fit tw-flex tw-justify-center tw-items-center tw-border tw-border-primary3 tw-mx-auto tw-rounded-xl tw-p-1">
                                                        <Link className="action-button tw-w-fit tw-bg-primary3 tw-text-white tw-py-1 tw-px-4 tw-mx-auto tw-rounded-full tw-no-underline" href={pathDetail(item?.id)}>
                                                            Detail
                                                        </Link>
                                                        <Divider orientation="vertical" flexItem sx={{ borderWidth: "0.px", borderColor: "#2B7FD4", margin: "0 0.25rem" }} />
                                                        <Link className="action-button tw-w-fit tw-bg-yellow-500 tw-text-white tw-py-1 tw-px-4 tw-mx-auto tw-rounded-full" href={pathEdit(item?.id)}>
                                                            Edit
                                                        </Link>
                                                    </div>
                                                </TableCell>
                                                    ...
                                                    <div className="action-button-wrapper tw-w-fit tw-flex tw-justify-center tw-items-center tw-border tw-border-primary3 tw-mx-auto tw-rounded-xl tw-p-1">
                                                        {
                                                            item?.detail?.status !== "banned" ?
                                                                <ActionButton title="Disqualify Participant" text="Apakah anda ingin mendiskualifikasi peserta?" link={`/back-office/monitor/${item?.detail?.exam_id}/start-exam/${item?.hash}`} buttonText="Disqualify" colorButton="red" status={status} disabled={data?.started_at ? false : true} />
                                                                :
...

                                            {
                                                ...
                                                <TableCell align='center'>
                                                    <div className="action-button-wrapper tw-w-fit tw-flex tw-justify-center tw-items-center tw-border tw-border-primary3 tw-mx-auto tw-rounded-xl tw-p-1">
                                                        <button
                                                            disabled={selectedParticipants.map(item => item.hash).includes(item?.hash) ? true : false}
                                                            className={`action-button tw-w-fit tw-bg-green-500 tw-text-white tw-py-1 tw-px-4 tw-mx-auto tw-rounded-full ${selectedParticipants.map(item => item.hash).includes(item?.hash) ? 'tw-cursor-not-allowed tw-opacity-40' : ''}`}
                                                            onClick={() => {
                                                                handleSelectParticipant(item?.hash)
                                                            }}>
                                                            Add
                                                        </button>
                                                    </div>
