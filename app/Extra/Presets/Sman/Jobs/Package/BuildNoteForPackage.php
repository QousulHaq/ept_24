...
namespace App\Extra\Presets\Sman\Jobs\Package;
...
class BuildNoteForPackage extends BaseJob implements ShouldQueue
{
    ...
    {
        if (! $this->package->preset instanceof Sman || ! is_null($this->package->parent_id)) {
            return false;
        }

        $this->checkSubpackage($this->package->children->firstWhere('config.title', Sman::SECTION_KOGNITIF));
        $this->checkSubpackage($this->package->children->firstWhere('config.title', Sman::SECTION_PENALARAN));
        $this->checkSubpackage($this->package->children->firstWhere('config.title', Sman::SECTION_LITERASI));
