...
namespace App\Extra\Presets\Cpns\Jobs\Package;
...
class BuildNoteForPackage extends BaseJob implements ShouldQueue
{
    ...
    {
        if (! $this->package->preset instanceof Cpns || ! is_null($this->package->parent_id)) {
            return false;
        }

        $this->checkSubpackage($this->package->children->firstWhere('config.title', Cpns::SECTION_TWK));
        $this->checkSubpackage($this->package->children->firstWhere('config.title', Cpns::SECTION_TIU));
        $this->checkSubpackage($this->package->children->firstWhere('config.title', Cpns::SECTION_TKP));
